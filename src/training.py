...
def train_model(X,y,target_names,dataset_name):
    ...
    for measure in parameters_dict['measures_lst']:
            ...
            if(k == parameters_dict['k_lst'][-1] and measure != 'None'):
                ...
                for p in parameters_dict['p_lst']:
                    ...
                    for x in range(magic_number):
                        ...
                        scores = custom_cross_val_score(obj,X,y,cv, measure, p)
                ...
                parameters_dict[new_key].extend([processing_p_time,acc_p,acc_p_std])

            tmp_proc_time = []
            for x in range(magic_number): # 30 times for statistical relevance
                # Train + Test
                start_time = time.time()
                scores = custom_cross_val_score(obj,X,y,cv, measure, parameters_dict['p_lst'][0])
        ...
        # print(f'Processing Time: {parameters_dict[measure][0]} - M Number: {magic_number}')
